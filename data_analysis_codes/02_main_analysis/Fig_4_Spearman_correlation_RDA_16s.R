# 11/09/2024 (updated from 03/05/2024)
# Sanjit Debnath
...
alpha_estimates_7 <- estimate_richness(ps_rarefy_7, measures = c("Chao1", "Shannon"))
...
shapiro.test(alpha_estimates_7$Salinity_ppm_avg) #  p-value < 2.2e-16
...
p1_p <- alpha_estimates_7 %>% 
  ggscatter(y = "Shannon", x = "Salinity_ppm_avg",
            ylab = "Shannon", xlab = "Salinity (ppm)",
...
rda.pang.f <- ps.pang %>%
  ...
  ord_calc(
    constraints = c("Salinity_ppm_avg", "Temperature_C_avg", "pH_avg", "DO_mg_l_avg"),
    method = "RDA"
  ) %>% 
  ord_plot(
    axes = c(1, 2),
...
#present in any sample after random subsampling
alpha_estimates_7t <- estimate_richness(ps_rarefy_7t, measures = c("Chao1", "Shannon"))
