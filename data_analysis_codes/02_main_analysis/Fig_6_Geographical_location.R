...
pt.up.c <- ggplot(data = alpha_estimates_2, aes(y = Chao1, x = Upazila)) +
...
#### Perform Kw test----
kruskal.test(Chao1 ~ Upazila, data = alpha_estimates_2)
#data:  Chao1 by Upazila
#Kruskal-Wallis chi-squared = 18.247, df = 2, p-value = 0.0001091

# perform Dunn test (https://www.youtube.com/watch?v=pyLQmUfrel8)
stat.test.ptc <- dunn_test(Chao1 ~ Upazila, data = alpha_estimates_2, p.adjust.method = "BH")

# add the value on the plotBox plot
stat.test.ptc <- stat.test.ptc %>% add_xy_position(x = "Upazila")
pt.up.c2 <- pt.up.c + stat_pvalue_manual(stat.test.ptc, label = "p.adj.signif", 
...
## B5. Beta diversity (weighted unifrac)----
...
## D5. Compositional barplot Geographical location----
# pangasius and tilapia combined
pseq_pt2 <- pseq_pt %>% 
  tax_fix()
...
### Richness----
# Chao1: An estimator of total species richness that accounts for the presence of rare species.
pt.up.c.18s <- ggplot(data = alpha_estimates_2.18s, aes(y = Chao1, x = Upazila)) +
  geom_boxplot(aes(color = Upazila)) +
  geom_jitter(aes(color = Upazila), width = 0.2, alpha = 0.8, size = 2, shape = 16) +  # Increase size of jitter points, set shape to round
  #geom_point() +
  scale_color_manual(values = upazila.colors2) +
...
#### Perform Kw test----
kruskal.test(Chao1 ~ Upazila, data = alpha_estimates_2.18s)
#data:  Chao1 by Upazila
#Kruskal-Wallis chi-squared = 50.899, df = 2, p-value = 8.861e-12

# perform Dunn test (https://www.youtube.com/watch?v=pyLQmUfrel8)
stat.test.ptc.18s <- dunn_test(Chao1 ~ Upazila, data = alpha_estimates_2.18s, p.adjust.method = "BH")

# add the value on the plotBox plot
stat.test.ptc.18s <- stat.test.ptc.18s %>% add_xy_position(x = "Upazila")
pt.up.c2.18s <- pt.up.c.18s + stat_pvalue_manual(stat.test.ptc.18s, label = "p.adj.signif", 
                                                 step.increase = 0.05, tip.length = 0.005, 
                                                 size = 8)  # Adjust the size of the asterisk here
#hide.ns = TRUE)
pt.up.c2.18s
...
## B6. Beta diversity (weighted unifrac)----
# wunifrac:weighted UniFrac took about #04:33 - 04:45 = 12 minutes to calculate the distance
...
library(microeco)
# LefSe with microeco
dataset.up <- phyloseq2meco(pseq_pt2)
lda.up.fam <- trans_diff$new(dataset = dataset.up, 
                             method = "lefse", 
                             group = "Upazila", 
                             taxa_level = "Family",
                             ...
                             p_adjust_method = "BH",
                             lefse_subgroup = NULL) 
lda.up.fam # After P value adjustment, 316 taxa found significant

##### Bar plot----
lda.up.fam$plot_diff_bar(threshold = 3)
up.fam <- lda.up.fam$plot_diff_bar(use_number = 1:20, width = 0.8)
up.fam

##### Relative abundance plot----
up.fam.bar <- lda.up.fam$plot_diff_abund(use_number = 1:20, #coord_flip = FALSE,
                                         add_sig = TRUE, 
                                         text_y_size = 12)
up.fam.bar
