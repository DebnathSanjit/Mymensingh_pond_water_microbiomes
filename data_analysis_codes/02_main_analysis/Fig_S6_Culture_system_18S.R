...
crop.c.18s <- ggplot(data = alpha.estimate.mono.18s, aes(y = Chao1, x = Crop)) +
...
#### Perform Kw test----
kruskal.test(Chao1 ~ Crop, data = alpha.estimate.mono.18s)
# data:  Chao1 by Crop
# Kruskal-Wallis chi-squared = 14.99, df = 1, p-value = 0.0001081
...
stat.test.crop.c.18s <- dunn_test(Chao1 ~ Crop, data = alpha.estimate.mono.18s, p.adjust.method = "BH")

# add the value on the plotBox plot
stat.test.crop.c.18s <- stat.test.crop.c.18s %>% add_xy_position(x = "Crop")
crop.c2.18s <- crop.c.18s + stat_pvalue_manual(stat.test.crop.c.18s, label = "p.adj.signif", 
...
### 1. Richness----
# Chao1: An estimator of total species richness that accounts for the presence of rare species.
cs.p.c.18s <- ggplot(data = alpha_estimates_2.3.p.18s, aes(y = Chao1, x = Culture_system)) +
...
#### Perform Kw test----
kruskal.test(Chao1 ~ Culture_system, data = alpha_estimates_2.3.p.18s)
# data:  Chao1 by Culture_system
# Kruskal-Wallis chi-squared = 14.258, df = 1, p-value = 0.0001593

##### Perform Dunn test---- 
#(https://www.youtube.com/watch?v=pyLQmUfrel8)
# Dunn test is not necessary but I'm doing it to add the p value on plot. Furthermore, it doesn't change the p value
stat.test.cs.p.c.18s <- dunn_test(Chao1 ~ Culture_system, data = alpha_estimates_2.3.p.18s, p.adjust.method = "BH")

# add the value on the plotBox plot
stat.test.cs.p.c.18s <- stat.test.cs.p.c.18s %>% add_xy_position(x = "Culture_system")
cs.p.c2.18s <- cs.p.c.18s + stat_pvalue_manual(stat.test.cs.p.c.18s, label = "p.adj.signif", 
                                               step.increase = 5, tip.length = 0.005, 
                                               size = 8)  # Adjust the size of the asterisk here
#hide.ns = TRUE)
cs.p.c2.18s
...
### 1. Richness----
# Chao1: An estimator of total species richness that accounts for the presence of rare species.
cs.t.c.18s <- ggplot(data = alpha_estimates_2.3.t.18s, aes(y = Chao1, x = Culture_system)) +
  geom_boxplot(aes(color = Culture_system)) +
  geom_jitter(aes(color = Culture_system), width = 0.2, alpha = 0.8, size = 2, shape = 16) +  # Increase size of jitter points, set shape to round
  #geom_point() +
  scale_color_manual(values = culture_system,
                     labels = c("Tilapia-monoculture" = "TM",
                                "Tilapia-polyculture" = "TP")) +
  theme(
    plot.title = element_text(hjust = 0, size = 16, margin = margin(b = 15)), # Increase margin for plot title
    axis.text.x = element_text(angle = 0, vjust = 0.5, hjust = 0.5, size = 22, margin = margin(t = 5)),
...
#### Perform Kw test----
kruskal.test(Chao1 ~ Culture_system, data = alpha_estimates_2.3.t.18s)
# data:  Chao1 by Culture_system
# Kruskal-Wallis chi-squared = 0.17105, df = 1, p-value = 0.6792
...
## Final plot----
fig_s5 <- grid.arrange(cs.barplot.18s,
                       cs.all.18s,
                       venn.18s,
