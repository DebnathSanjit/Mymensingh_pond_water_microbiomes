...
# This script is for microbial (prokaryotic and microeukaryotic) seasonal dynamics including shannon diversity, glm, beta diversity and harmonic regression at family rank with a relative abundance > 1%
# Load Libraries
...
p.sm.s <- ggplot(data = alpha_estimates_2.1, aes(y = Shannon, x = Sampling_months)) +
  ...
  #geom_point() +
  scale_color_manual(values = season.colors2) +
  theme(
...
p.s.we <- p_wuni %>%
  ...
  scale_colour_manual(values = season.colors3) +
  scale_fill_manual(values = season.colors3) +
  theme(
...
# Final alpha plot
season.alpha2 <- cowplot::plot_grid(season.alpha,
                                    legend.season,
                                    ncol = 1,
                                    rel_heights = c(2, 0.2))
season.alpha2 # fix the legend in inkscape

# combine beta diversity
season.beta <- cowplot::plot_grid(
                        ...
                        ncol = 1, labels = c("E", "F", "G", "H"))
season.beta
## get legend
legend.beta <- get_legend(p.s.we)

# Final beta plot
season.beta2 <- cowplot::plot_grid(season.beta,
                                   legend.beta, 
                                   ncol = 1,
                                   rel_heights = c(2, 0.2))
season.beta2 #The legend doesn't add due to some reason. save this plot seperately and add the legend in inkscape
...
# Combined seasonal dynamics
comb.season <- cowplot::plot_grid(season.alpha2,
                                  season.beta2, # add legend seperately
                                  season.hr,
                                  rel_widths = c(3.75, 2.5, 3.75),
                                  ncol = 3)
comb.season
...
legend.16s <- cowplot::plot_grid(#legend.season,
                                 legend.pang.fam,
                                 legend.tila.fam, rel_heights = c(4,6),
                                 ncol = 1)
...
comb.seasonal.trend <- cowplot::plot_grid(comb.season,
                                          ...
                                          rel_widths = c(2, 0.4))
comb.seasonal.trend
# Save as 2970*2100 and give a final touch for legends in inkscape. 
#For beta diversity, add the legend in the bottom from a plot
