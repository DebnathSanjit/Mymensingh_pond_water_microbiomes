...
p.up.c <- ggplot(data = alpha_estimates_2.1, aes(y = Chao1, x = Upazila)) +
  ...
  labs (y = "Chao1", # Add y-axis title to the plot
        x = "Upazila")+
  #title = "a) Pangasius pond") +
  scale_x_discrete(labels = function(x) gsub("_", "\n", x)) # Insert line break in category names
p.up.c

#### Perform Kw test----
kruskal.test(Chao1 ~ Upazila, data = alpha_estimates_2.1)
#data:  Chao1 by Upazila
#Kruskal-Wallis chi-squared = 9.94, df = 1, p-value = 0.001617

##### Perform Dunn test (https://www.youtube.com/watch?v=pyLQmUfrel8)----
stat.test.pvc <- dunn_test(Chao1 ~ Upazila, data = alpha_estimates_2.1, p.adjust.method = "BH")
...
stat.test.pvc <- stat.test.pvc %>% add_xy_position(x = "Upazila")
p.up.c2 <- p.up.c + stat_pvalue_manual(stat.test.pvc, label = "p.adj.signif", 
...
### Richness----
# Chao1: An estimator of total species richness that accounts for the presence of rare species.
t.up.c <- ggplot(data = alpha_estimates_2.2, aes(y = Chao1, x = Upazila)) +
  geom_boxplot(aes(color = Upazila)) +
  geom_jitter(aes(color = Upazila), width = 0.2, alpha = 0.8, size = 2, shape = 16) +  # Increase size of jitter points, set shape to round
  #geom_point() +
  scale_color_manual(values = upazila.colors,
  ...
  theme(
    ...
    legend.text = element_text(size = 18),
    panel.grid = element_blank()) +
  guides(fill = guide_legend(nrow = 1)) + # Set number of columns in the legend # Set number of columns in the legend
  labs (y = "Chao1", # Add y-axis title to the plot
        x = "Upazila")+
  #title = "e) Tilapia pond") + 
  scale_x_discrete(labels = function(x) gsub("_", "\n", x))
t.up.c

#### Perform Kw test----
kruskal.test(Chao1 ~ Upazila, data = alpha_estimates_2.2)
#data:  Chao1 by Upazila
#Kruskal-Wallis chi-squared = 1.9956, df = 1, p-value = 0.1578

# perform Dunn test (https://www.youtube.com/watch?v=pyLQmUfrel8)
stat.test.tuc <- dunn_test(Chao1 ~ Upazila, data = alpha_estimates_2.2, p.adjust.method = "BH")

# add the value on the plotBox plot
stat.test.tuc <- stat.test.tuc %>% add_xy_position(x = "Upazila")
t.up.c2 <- t.up.c + stat_pvalue_manual(stat.test.tuc, label = "p.adj.signif", 
                                       step.increase = 0.05, tip.length = 0.005, 
                                       size = 8)  # Adjust the size of the asterisk here
#hide.ns = TRUE)
t.up.c2
...
### Richness----
# Chao1: An estimator of total species richness that accounts for the presence of rare species.
p.up.c.18s <- ggplot(data = alpha_estimates_2.1.18s, aes(y = Chao1, x = Upazila)) +
  geom_boxplot(aes(color = Upazila)) +
  geom_jitter(aes(color = Upazila), width = 0.2, alpha = 0.8, size = 2, shape = 16) +  # Increase size of jitter points, set shape to round
  #geom_point() +
  scale_color_manual(values = upazila.colors,
...
#### Perform Kw test----
kruskal.test(Chao1 ~ Upazila, data = alpha_estimates_2.1.18s)
#data:  Chao1 by Upazila
#Kruskal-Wallis chi-squared = 10.08, df = 1, p-value = 0.001499

##### Perform Dunn test (https://www.youtube.com/watch?v=pyLQmUfrel8)----
stat.test.pvc.18s <- dunn_test(Chao1 ~ Upazila, data = alpha_estimates_2.1.18s, p.adjust.method = "BH")
# actually for two variables i don't need dunn test but to automatically add the value on 
# plot, i did it. It doesn't change the value

# add the value on the plotBox plot
stat.test.pvc.18s <- stat.test.pvc.18s %>% add_xy_position(x = "Upazila")
p.up.c2.18s <- p.up.c.18s + stat_pvalue_manual(stat.test.pvc.18s, label = "p.adj.signif", 
                                               step.increase = 0.05, tip.length = 0.005, 
                                               size = 8,  # Adjust the size of the asterisk here
                                               hide.ns = TRUE)
p.up.c2.18s
...
### Richness----
# Chao1: An estimator of total species richness that accounts for the presence of rare species.
t.up.c.18s <- ggplot(data = alpha_estimates_2.2.18s, aes(y = Chao1, x = Upazila)) +
  geom_boxplot(aes(color = Upazila)) +
  geom_jitter(aes(color = Upazila), width = 0.2, alpha = 0.8, size = 2, shape = 16) +  # Increase size of jitter points, set shape to round
  #geom_point() +
  scale_color_manual(values = upazila.colors,
...
#### Perform Kw test----
kruskal.test(Chao1 ~ Upazila, data = alpha_estimates_2.2.18s)
#data:  Chao1 by Upazila
#Kruskal-Wallis chi-squared = 22.922, df = 1, p-value = 1.688e-06

# perform Dunn test (https://www.youtube.com/watch?v=pyLQmUfrel8)
stat.test.tuc.18s <- dunn_test(Chao1 ~ Upazila, data = alpha_estimates_2.2.18s, p.adjust.method = "BH")

# add the value on the plotBox plot
stat.test.tuc.18s <- stat.test.tuc.18s %>% add_xy_position(x = "Upazila")
t.up.c2.18s <- t.up.c.18s + stat_pvalue_manual(stat.test.tuc.18s, label = "p.adj.signif", 
                                               step.increase = 0.05, tip.length = 0.005, 
                                               size = 8)  # Adjust the size of the asterisk here
#hide.ns = TRUE)
t.up.c2.18s
